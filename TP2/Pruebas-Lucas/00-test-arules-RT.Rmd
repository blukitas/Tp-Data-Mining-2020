...
bin_eq_freq <- discretize(df_user_features$tiempo_primera_rpta_RT_m,"equalfreq", 5)
bin_eq_freq$tiempo_primera_rpta_RT_m = df_user_features$tiempo_primera_rpta_RT_m

# Por cada bin calculamos la media y reemplazamos en el atributo suavizado
for(bin in 1:5){
  bin_eq_freq$suavizado[ bin_eq_freq$X==bin] = mean(bin_eq_freq$tiempo_primera_rpta_RT_m[ bin_eq_freq$X==bin])
}

table(bin_eq_freq$suavizado)

# grafico Sepal.Width ordenado de menor a mayor
plot(sort(df_user_features$tiempo_primera_rpta_RT_m) , type = "p", col="red", 
     ylab = "tiempo_primera_rpta_RT_m", xlab = "Observaciones", main = "Dato original vs suavizado")
...
# df_user_features$tiempo_primera_rpta_RT_m
```
```{r}
hist(df_user_features$tiempo_primera_rpta_RT_m)

df_user_features$cat_1_resp <- arules::discretize(df_user_features$tiempo_primera_rpta_RT_m, breaks=5, labels=c("ahora","rapida", "tranqui", "hace un ratín", "desde el pasado") )
 barplot(table(df_user_features$cat_1_resp)) 

 df_user_features$cat_1_resp_2 <- arules::discretize(df_user_features$tiempo_primera_rpta_RT_m, method = "fixed", breaks=c(-Inf, 0.8, 1.75, 2.25, 2.8, Inf), labels=c("ahora","rapida", "tranqui", "Hace rato", "desde el pasado") )
 barplot(table(df_user_features$cat_1_resp_2)) 
 # 0.8 => 6' 20"
 # 1.75 => 56' 15"
 # 2.25 => 117.82' => 2hs 58'
...
```{r}
bin_eq_freq <- discretize(df_user_features$retweet_favorite_count,"equalfreq", 5)
bin_eq_freq$retweet_favorite_count = df_user_features$retweet_favorite_count
...
# grafico Sepal.Width ordenado de menor a mayor
plot(sort(df_user_features$retweet_favorite_count) , type = "p", col="red", 
...
# df_user_features$retweet_favorite_count
table(bin_eq_freq$suavizado)
...
```{r}
bin_eq_freq <- discretize(df_user_features$retweet_retweet_count,"equalfreq", 4)
bin_eq_freq$retweet_retweet_count = df_user_features$retweet_retweet_count
...
# grafico Sepal.Width ordenado de menor a mayor
plot(sort(df_user_features$retweet_retweet_count) , type = "p", col="red", 
     ylab = "listed_count", xlab = "Observaciones", main = "Dato original vs suavizado")
# Agrego la serie de la variable media 
lines(sort(bin_eq_freq$suavizado),
      type = "p", col="blue")
...
bin_eq_freq <- discretize(df_user_features$RT_followers_friends_ratio,"equalfreq", 4)
bin_eq_freq$RT_followers_friends_ratio = df_user_features$RT_followers_friends_ratio

# Por cada bin calculamos la media y reemplazamos en el atributo suavizado
for(bin in 1:4){
  bin_eq_freq$suavizado[ bin_eq_freq$X==bin] = mean(bin_eq_freq$RT_followers_friends_ratio[ bin_eq_freq$X==bin])
}

# grafico Sepal.Width ordenado de menor a mayor
plot(sort(df_user_features$RT_followers_friends_ratio) , type = "p", col="red", 
     ylab = "listed_count", xlab = "Observaciones", main = "Dato original vs suavizado")
# Agrego la serie de la variable media 
lines(sort(bin_eq_freq$suavizado),
      type = "p", col="blue")
...
df_user_features$cat_adherencia = arules::discretize(df_user_features$retweet_favorite_count,method = "fixed", c(-Inf,0.5, 1.2, 2.2, Inf), labels = c("baja", "intermedia", "alta", "muy_alta"))
...
rules = apriori(trans, parameter=list(target="rule",  minlen=2, support=0.02, confidence=0.05))
#print(rules)
